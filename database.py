...
def query_all_entities() -> list:
    conn = get_connection(); cursor = conn.cursor(); cursor.execute("SELECT * FROM entities"); rows = cursor.fetchall(); conn.close(); return [dict(r) for r in rows]

def query_entity_by_id(entity_id: str) -> Optional[dict]:
    conn = get_connection(); cursor = conn.cursor(); cursor.execute("SELECT * FROM entities WHERE id = ?", (entity_id,)); row = cursor.fetchone(); conn.close(); return dict(row) if row else None
...
def get_pending_task() -> dict | None:
    """使用 RETURNING 实现完美的原子出队操作，彻底消灭并发死锁"""
    conn = get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("""
...
def get_smart_rag_context(query: str) -> str:
    conn = get_connection(); cursor = conn.cursor()
    cursor.execute("SELECT id, name, aliases_json FROM entities"); all_entities = cursor.fetchall()
    ...
    if not matched_entity_ids:
        cursor.execute("SELECT title, date, summary FROM events ORDER BY date DESC LIMIT 30"); events = cursor.fetchall(); conn.close()
